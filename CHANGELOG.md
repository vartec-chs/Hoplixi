...
- В desktop layout добавлено ручное сворачивание/разворачивание панели
  `DashboardDrawer` через иконки `panelRightClose` и `panelRightOpen`, включая
  случаи, когда правая панель закрыта; overflow при анимации ширины исправлен
  клипованием содержимого панели, а правая панель теперь синхронно реагирует на
  изменение ширины левой панели без резкого смещения. Кнопка открытия в
  свернутом состоянии выровнена по позиции кнопки закрытия в заголовке. На
  desktop-ширине без места для обеих панелей при открытой правой панели теперь
  остаётся свернутый rail с выключенной кнопкой открытия вместо полного скрытия
  `DashboardDrawer`.
