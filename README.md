...
- Выбранный при создании алгоритм сохраняется в `store_key.json`.
- При открытии хранилища приложение использует сохранённый cipher.
- Для старых хранилищ (или неконсистентного конфига) используется
  fallback-подбор доступных cipher, после чего рабочий вариант записывается в
  конфиг.
...
генерации ключа и корректного открытия хранилища. store_manifest.json - содержит
метаданные хранилища (uuid, дату обновления). store_name.hplxdb - зашифрованная
база данных SQLite3 Multiple Ciphers. attachments_decrypted/ - временная папка
для расшифрованных файлов (может не существовать). attachments/ - папка для
