...
@DriftAccessor(
  tables: [VaultItems, FileItems, FileMetadata, Categories, Tags, ItemTags],
)
class FileFilterDao extends DatabaseAccessor<VaultDB>
  ...
  Expression<bool> _buildWhere(FileFilter filter) {
    ...
    if (filter.minFileSize != null) {
      whereExpr &= fileMetadata.fileSize.isBiggerOrEqualValue(
        filter.minFileSize!,
      );
    }
    if (filter.maxFileSize != null) {
      whereExpr &= fileMetadata.fileSize.isSmallerOrEqualValue(
        filter.maxFileSize!,
      );
    }

    if (filter.availabilityStatus != null) {
      whereExpr &= fileMetadata.availabilityStatus.equalsValue(
        filter.availabilityStatus!,
      );
    }
    if (filter.integrityStatus != null) {
      whereExpr &= fileMetadata.integrityStatus.equalsValue(
        filter.integrityStatus!,
      );
    }

    if (filter.missingDetectedAfter != null) {
      whereExpr &= fileMetadata.missingDetectedAt.isBiggerOrEqualValue(
        filter.missingDetectedAfter!,
      );
    }
    if (filter.deletedAfter != null) {
      whereExpr &= fileMetadata.deletedAt.isBiggerOrEqualValue(
        filter.deletedAfter!,
      );
    }
    if (filter.lastIntegrityCheckAfter != null) {
      whereExpr &= fileMetadata.lastIntegrityCheckAt.isBiggerOrEqualValue(
        filter.lastIntegrityCheckAfter!,
      );
    }
    if (filter.lastIntegrityCheckBefore != null) {
      whereExpr &= fileMetadata.lastIntegrityCheckAt.isSmallerOrEqualValue(
        filter.lastIntegrityCheckBefore!,
      );
    }
