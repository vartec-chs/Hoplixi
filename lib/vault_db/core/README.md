...
- `vault_db.dart` объединяет таблицы, DAOs, индексы и триггеры в единую
  Drift-конфигурацию.
- Класс `VaultDB` должен оставаться тонким: orchestration и прикладная логика
...
#### [VaultRepositories](./repositories/vault_repositories.dart)

Единая точка создания и хранения репозиториев поверх `VaultDB`.

- Собирает все репозитории ядра в одном объекте, чтобы не создавать их вручную в
  каждом сервисе.
- Дает сервисам доступ к нужным DAO-слоям через предсказуемый интерфейс
  `repositories.<name>`.
- Упрощает передачу зависимостей: сервисы получают один контейнер с уже готовыми
  репозиториями вместо набора отдельных конструкторов.

#### [VaultEntityServices](./services/vault_entity_services.dart)

Контейнер для всех сервисов сущностей, работающих поверх репозиториев.

- Собирает типизированные CRUD-сервисы для конкретных vault-сущностей
  (`password`, `note`, `apiKey` и т.д.).
- Использует `VaultEntityServiceDeps`, чтобы передать общие зависимости для
  истории, связей, состояния элементов и доступа к `VaultDB`.
- Нужен как единая точка входа для прикладного слоя, когда требуется работать не
  с голыми репозиториями, а с готовой бизнес-логикой по типам сущностей.
...
- [VaultRepositories](./repositories/vault_repositories.dart) выступает
  контейнером, который лениво создаёт и хранит все репозитории ядра.
- Репозитории скрывают детали реализации Drift внутри себя и собирают данные в
  более удобные доменные формы.
- Их используют сервисы из `services/`, чтобы не зависеть от отдельных DAO и
  держать доступ к данным централизованным.
...
- [VaultEntityServices](./services/vault_entity_services.dart) — контейнер
  готовых сервисов сущностей, который создаётся поверх `VaultRepositories` и
  даёт прикладному слою доступ к операциям по конкретным типам данных.
