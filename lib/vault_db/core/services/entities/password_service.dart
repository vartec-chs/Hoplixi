...
import 'package:hoplixi/vault_db/core/validators/password_validator.dart';
import 'package:result_dart/result_dart.dart';

class PasswordService extends BaseVaultEntityService<PasswordRepository> {
  PasswordService({required super.deps, required super.repository});
      ...
      return await db.transaction(() async {
        ...
        final createdView = createdViewResult.getOrThrow().fold(
          ...
          () => throw DBCoreError.notFound(
            entity: 'password',
            id: itemId,
            message: 'Failed to retrieve created Password: $itemId',
          ),
        );
  ...
  Future<DBResult<Unit>> update(PatchPasswordDto dto) async {
    final validationError = validatePatchPassword(dto);
    if (validationError != null) return Failure(validationError);
      ...
      return await db.transaction(() async {
        ...
        if (oldView == null) {
          throw DBCoreError.notFound(
            entity: 'password',
            id: itemId,
            message: 'Password not found for update: $itemId',
          );
        }
