...
class CertificateHistoryDao extends DatabaseAccessor<MainStore>
  ...
  Future<List<CertificateHistoryData>> getCertificateHistoryByHistoryIds(List<String> historyIds) {
    if (historyIds.isEmpty) return Future.value(const []);
    return (select(certificateHistory)..where((tbl) => tbl.historyId.isIn(historyIds))).get();
  }

  Future<Map<String, CertificateHistoryCardDataDto>> getCertificateHistoryCardDataByHistoryIds(List<String> historyIds) async {
    if (historyIds.isEmpty) return const {};

    final hasCertificatePemExpr = certificateHistory.certificatePem.isNotNull();
    final hasCertificateBlobExpr = certificateHistory.certificateBlob.isNotNull();
    final hasPrivateKeyExpr = certificateHistory.privateKey.isNotNull();
    final hasPrivateKeyPasswordExpr = certificateHistory.privateKeyPassword.isNotNull();
    ...
    return {
      for (final row in rows)
        row.read(certificateHistory.historyId)!: CertificateHistoryCardDataDto(
          certificateFormat: row.readWithConverter<CertificateFormat?, String>(certificateHistory.certificateFormat),
          keyAlgorithm: row.readWithConverter<CertificateKeyAlgorithm?, String>(certificateHistory.keyAlgorithm),
