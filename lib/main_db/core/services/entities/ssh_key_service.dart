...
class SshKeyService {
  ...
  Future<String> create(CreateSshKeyDto dto) async {
    return await db.transaction(() async {
      ...
      if (createdView == null) {
        throw Exception('Failed to retrieve created SshKey: $itemId');
      }
  ...
  Future<void> update(PatchSshKeyDto dto) async {
    await db.transaction(() async {
      final itemId = dto.item.itemId;

      final oldView = await repository.getViewById(itemId);
      if (oldView == null) {
        throw Exception('SshKey not found for update: $itemId');
      }
