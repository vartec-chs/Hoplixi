...
class LoyaltyCardService {
  ...
  Future<String> create(CreateLoyaltyCardDto dto) async {
    return await db.transaction(() async {
      ...
      if (createdView == null) {
        throw Exception('Failed to retrieve created LoyaltyCard: $itemId');
      }
  ...
  Future<void> update(PatchLoyaltyCardDto dto) async {
    await db.transaction(() async {
      final itemId = dto.item.itemId;

      final oldView = await repository.getViewById(itemId);
      if (oldView == null) {
        throw Exception('LoyaltyCard not found for update: $itemId');
      }
