...
class IdentityService {
  ...
  Future<String> create(CreateIdentityDto dto) async {
    return await db.transaction(() async {
      ...
      if (createdView == null) {
        throw Exception('Failed to retrieve created Identity: $itemId');
      }
  ...
  Future<void> update(PatchIdentityDto dto) async {
    await db.transaction(() async {
      final itemId = dto.item.itemId;

      final oldView = await repository.getViewById(itemId);
      if (oldView == null) {
        throw Exception('Identity not found for update: $itemId');
      }
