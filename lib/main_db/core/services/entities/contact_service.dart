...
class ContactService {
  ...
  Future<String> create(CreateContactDto dto) async {
    return await db.transaction(() async {
      ...
      if (createdView == null) {
        throw Exception('Failed to retrieve created Contact: $itemId');
      }
  ...
  Future<void> update(PatchContactDto dto) async {
    await db.transaction(() async {
      final itemId = dto.item.itemId;

      final oldView = await repository.getViewById(itemId);
      if (oldView == null) {
        throw Exception('Contact not found for update: $itemId');
      }
