...
class CryptoWalletService {
  ...
  Future<String> create(CreateCryptoWalletDto dto) async {
    return await db.transaction(() async {
      ...
      if (createdView == null) {
        throw Exception('Failed to retrieve created CryptoWallet: $itemId');
      }
  ...
  Future<void> update(PatchCryptoWalletDto dto) async {
    await db.transaction(() async {
      final itemId = dto.item.itemId;

      final oldView = await repository.getViewById(itemId);
      if (oldView == null) {
        throw Exception('CryptoWallet not found for update: $itemId');
      }
