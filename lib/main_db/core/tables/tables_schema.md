...
`sortOrder`, `createdAt`, `modifiedAt`, `snapshotCreatedAt`. Notes: Хранит
...
`aggregateSha256Hash`, `pageCount`. Notes: `aggregateSha256Hash` — хэш текущей
агрегированной версии документа/страниц. enum DocumentType { passport, idCard,
driverLicense, contract, invoice, receipt, certificate, insurance, tax,
medical, legal, financial, other, }

### document_pages

Страницы документа. Columns: `id`, `documentId`, `metadataId`, `pageNumber`,
`extractedText`, `pageSha256Hash`, `isPrimary`, `usedCount`, `createdAt`,
...
`isCurrent`, `createdAt`, `modifiedAt`. Notes: Ссылка на историю изменений
`itemHistoryId`. Snapshot-метаданные файлов хранятся на уровне страниц версии
через `document_version_pages.metadataHistoryId`. Уникальность
...
- **item_link_history**: `id`, `historyId`, `sourceLinkId`, `sourceItemId`,
  ...
  `createdAt`, `modifiedAt`, `snapshotCreatedAt`.
