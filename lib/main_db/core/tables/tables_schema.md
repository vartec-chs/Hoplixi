...
`vault_items` на момент действия `action`. enum VaultItemHistoryAction {
created, updated, archived, restored, deleted, recovered, favorited,
unfavorited, pinned, unpinned, }
...
OCR текст документа. enum DocumentType { passport, idCard, driverLicense,
contract, invoice, receipt, certificate, insurance, tax, medical, legal,
financial, other, }
...
Версии документа. Columns: `id`, `documentId`, `versionNumber`,
`documentType`, `documentTypeOther`, `aggregatedText`, `aggregateHash`,
`pageCount`, `isCurrent`, `createdAt`, `modifiedAt`. Notes: связь с
`vault_items` для документа. Snapshot-метаданные файлов хранятся на уровне
страниц версии через `document_version_pages.metadataHistoryId`.
Уникальность `(documentId, versionNumber)`, частичный unique-index ограничивает
одну активную версию на документ.

### document_version_pages

Страницы версии документа. Columns: `id`, `versionId`, `metadataHistoryId`,
`pageNumber`, `extractedText`, `pageHash`, `isPrimary`, `createdAt`. Notes:
связь с `document_versions` и snapshot-метаданными страницы в
`file_metadata_history`. Уникальность
`(versionId, pageNumber)`, частичный unique-index ограничивает одну
primary-страницу на версию.
...
`filePath`, `mimeType`, `fileSize`, `fileHash`. Notes: Реальный путь и хеш
файла.
...
`snapshotCreatedAt`. Notes: `metadataId` не является FK, чтобы snapshot
