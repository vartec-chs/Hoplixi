...
enum CardType {
  debit, // Дебетовая карта, которая списывает средства непосредственно с банковского счёта владельца.
  credit, // Кредитная карта, которая позволяет владельцу занимать деньги у банка в рамках установленного кредитного лимита.
  prepaid, // Предоплаченная карта, которая требует предварительного пополнения средств перед использованием.
  virtual, // Виртуальная карта, которая существует только в цифровом виде и обычно используется для онлайн-покупок.
  other,
}
