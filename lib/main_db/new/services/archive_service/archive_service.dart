...
class ArchiveService {
  ...
  Future<rd.ResultDart<String, AppError>> archiveStore(
    String storePath,
    String outputPath, {
    String? password,
    ...
    try {
      ...
      ArchiveIsolateResult? result;

      await for (final message in receivePort) {
        if (message is ArchiveProgressMessage) {
          onProgress?.call(message.current, message.total, message.fileName);
        } else if (message is ArchiveIsolateResult) {
          result = message;
          break;
        }
      }
      ...
      return rd.Failure(
        AppError.archive(
          code: ArchiveErrorCode.archiveFailed,
      ...
      return rd.Failure(
        e.toFileSystemAppError(
          ...
          stackTrace: s,
          data: {'outputPath': outputPath, 'storePath': storePath},
        ),
      );
    }
  }

  Future<rd.ResultDart<String, AppError>> unarchiveStore(
    String archivePath, {
    String? password,
    String? basePath,
    ...
    try {
      ...
      if (result?.success != true) {
        ...
        if (result?.isInvalidPassword == true) {
          return rd.Failure(
            AppError.archive(
              code: ArchiveErrorCode.invalidPassword,
        ...
        return rd.Failure(
          AppError.archive(
            code: ArchiveErrorCode.unarchiveFailed,
...
void _archiveInIsolate(ArchiveParams params) async {
  try {
    ...
    params.sendPort.send(ArchiveIsolateResult.success(params.outputPath));
  } catch (e) {
    params.sendPort.send(ArchiveIsolateResult.error(e.toString()));
  }
}
...
Future<void> _archiveWithoutPasswordStreamingIsolate({
  ...
  try {
    for (var index = 0; index < files.length; index++) {
      ...
      sendPort.send(
        ArchiveProgressMessage(index + 1, totalFiles, relativePath),
      );
...
void _unarchiveInIsolate(UnarchiveParams params) async {
  try {
      ...
      for (final file in archive) {
        ...
        if (file.isFile) {
          ...
          params.sendPort.send(
            ArchiveProgressMessage(currentFile, totalFiles, filename),
          );
    ...
    params.sendPort.send(ArchiveIsolateResult.success(params.targetPath));
  } catch (e) {
    final errorMessage = e.toString();
    final isInvalidPassword = _isPasswordError(errorMessage);
    params.sendPort.send(
      ArchiveIsolateResult.error(
        errorMessage,
