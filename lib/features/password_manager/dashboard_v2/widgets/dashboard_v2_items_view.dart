...
const _dashboardV2ItemAnimationDuration = Duration(milliseconds: 180);

final class DashboardV2ItemsView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DashboardV2ItemsView> createState() =>
      _DashboardV2ItemsViewState();
}

final class _DashboardV2ItemsViewState
    extends ConsumerState<DashboardV2ItemsView> {
  ...
  @override
  void didUpdateWidget(covariant DashboardV2ItemsView oldWidget) {
  ...
  Widget _itemTransition({
    ...
    return _DashboardV2ItemEntranceTransition(
      itemId: item.id,
      child: transition,
    );
  }
  ...
  void _performDiff(
    ...
    if (didMutate && newItems.isEmpty) {
      _isClearing = true;
      Timer(_dashboardV2ItemAnimationDuration, () {
  ...
  void _animateInsert(
    ...
    if (widget.viewMode.isGrid) {
      gridState?.insertItem(index, duration: _dashboardV2ItemAnimationDuration);
      return;
    }

    listState?.insertItem(index, duration: _dashboardV2ItemAnimationDuration);
  }
  ...
  void _animateRemove(
    ...
    if (widget.viewMode.isGrid) {
      gridState?.removeItem(
        index,
        builder,
        duration: _dashboardV2ItemAnimationDuration,
      );
      return;
    }
...
final class _DashboardV2ItemEntranceTransition extends StatelessWidget {
  const _DashboardV2ItemEntranceTransition({
