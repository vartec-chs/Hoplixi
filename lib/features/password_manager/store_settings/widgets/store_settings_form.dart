...
class _StoreSettingsFormState extends ConsumerState<StoreSettingsForm> {
  ...
  bool _showCompileOptions = false;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (state.pragmaCompileOptions.isNotEmpty) ...[
            const SizedBox(height: 8),
            InkWell(
              onTap: () {
                setState(() {
                  _showCompileOptions = !_showCompileOptions;
                });
              },
              borderRadius: BorderRadius.circular(12),
              child: Container(
            ...
            subtitle: RichText(
              text: TextSpan(
                ...
                children: [
                  const TextSpan(
                    text:
                        'Если опция отключена, раздел «Часто используемые» может работать менее точно или быть недоступен вовсе. ',
                  ),
                  const TextSpan(
                    text:
                  ...
                  const TextSpan(
                    text:
                        'Включить опцию можно в любой момент — после этого статистика начнёт собираться для всех последующих копирований.',
                  ),
                  const TextSpan(text: '\n\n'),
                  const TextSpan(
                    text:
                        'Опцию можно отключить, если вы часто копируете данные из хранилища и не хотите, чтобы это влияло на рейтинг элементов в разделе «Часто используемые», либо хотите уменьшить количество синхронизаций с облаком из-за частых изменений рейтинга.',
                  ),
                ],
              ),
            ),
