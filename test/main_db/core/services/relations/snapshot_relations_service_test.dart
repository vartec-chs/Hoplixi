...
import 'package:hoplixi/vault_db/core/services/relations/snapshot_relations_service.dart';
...
void main() {
  ...
  group('SnapshotRelationsService', () {
    test('snapshotCategoryForItem creates category history snapshot', () async {
      ...
      final history = await db.itemCategoryHistoryDao.getCategoryHistoryById(
        categoryHistoryId!,
      );
    ...
    test(
      'snapshotCategoryForItem returns null when categoryId is null',
      () async {
        final snapshotId = const Uuid().v4();
    ...
    test('snapshotTagsForItem creates tag history rows', () async {
      ...
      await service.snapshotTagsForItem(historyId: historyId, itemId: itemId);

      final count = await dataFactory.countTable(db.vaultItemTagHistory);
      expect(count, 2);

      final historyTags = await db.vaultItemTagHistoryDao
    ...
    test('snapshotLinksForItem creates item link history rows', () async {
      ...
      await service.snapshotLinksForItem(historyId: historyId, itemId: item1Id);

      final count = await dataFactory.countTable(db.itemLinkHistory);
      expect(count, 1);

      final historyLinks = await db.itemLinkHistoryDao
          .getLinksBySnapshotHistoryId(historyId);
    ...
    group('DbNotFoundError handling', () {
      test(
        'snapshotCategoryForItem returns null if category not in DB',
        () async {
          final result = await service.snapshotCategoryForItem(
    ...
    group('History Policies', () {
      test('isHistoryEnabled returns true by default', () async {
        final policyService = TestServiceFactory(
          db,
        ).createStoreHistoryPolicyService();
