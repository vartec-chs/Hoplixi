...
void main() {
  ...
  test('normalize pack and icon keys', () {
    ...
    expect(IconPackCatalogService.normalizePackKey('Пак<>:"/\\\\|?*'), 'пак');
    expect(
  ...
  test(
    'imports archive, strips shared root and ignores non-svg files',
    () async {
      ...
      expect(
        icons.map((entry) => entry.relativePath),
        containsAll(<String>['social/github.svg', 'social/twitter.svg']),
      );
      expect(
        icons.map((entry) => entry.key),
        contains('brand_pack/social/github'),
      );
      expect(
      ...
      expect(
        File(p.join(importedDir.path, 'manifest.json')).existsSync(),
        isTrue,
      );
      expect(
        File(p.join(importedDir.path, 'index.jsonl')).existsSync(),
        isTrue,
      );
      expect(
        File(
          p.join(importedDir.path, 'icons', 'social', 'github.svg'),
        ).existsSync(),
        isTrue,
      );
    },
  ...
  test('imports directory and strips shared root folder', () async {
    ...
    await Directory(
      p.join(sourceDirectory.path, 'folder-pack', 'social'),
  ...
  test(
    'supports pagination, query filtering and deterministic collision suffixes',
    () async {
      final archiveFile = await _createZipFile(
        directory: tempDir,
      ...
      final allIcons = await service.listIcons(
        packKey: result.packKey,
        limit: 20,
      );
      expect(
        allIcons.map((entry) => entry.key),
        contains('collision_pack/foo'),
      );
      expect(
        allIcons.map((entry) => entry.key),
        contains('collision_pack/foo_2'),
      );
